...
export interface RemoteAudioActivityTap extends RemoteAudioActivitySource {
   ...
   *  data (#1192). Additive — a lane that never calls it leaves presence unknown. */
...
export function createRemoteAudioActivityTap(options: {
  ...
  return {
    ready(): void {
      // Presence bookkeeping restarts with capture; the page's stream reports do NOT — they are a
      // fact about the meeting, not about our capture chain, and survive a capture restart.
    ...
    unavailable(): void {
      ...
      // this is the #1174 fail-closed state, byte-for-byte — `available: false` short-circuits the
      // monitor before any adapter runs.
      state = {
        available: false,
 ...
 * never an empty room.
 *
 * The silence adapter's only oracle is frame arrival, so a bot whose capture chain dies mid-meeting
 * (the #850 class, at 100% loss) hears nothing, lets the window expire and leaves a LIVE meeting as
 * `completed(left_alone)` — a recorded success. The page already knows remote streams are connected
 * and carrying data; this adapter is the Node-side consumer of that bit.
 *
 ...
 *   - the lane never reports presence (gmeet captures per channel — unknown ⇒ today's behavior);
 *   - the last report is staler than `stalenessMs` (a dead page must not hold a bot open);
 *   - no stream has been reported present recently (the room really did empty);
...
export function createDeafCaptureGuardAdapter(options: { stalenessMs?: number } = {}): AlonenessAdapter {
  ...
  return {
    ...
    evaluate(snapshot, now, windowMs): AlonenessVerdict {
      ...
      if (snapshot.streamsPresentAt === undefined) return 'alone';               // never any stream
      if (now - snapshot.streamsPresentAt > stalenessMs) return 'alone';         // streams are gone
