...
### Explicit schema convergence

`migrations.enabled` is off by default because admin-api converges its durable schema during
normal startup. Enable the Job only when an explicit pre-rollout convergence gate is required.
The Job always runs the **admin-api image** at the same `global.imageTag` as the release and invokes
`python -m admin_api.schema_runner`; it does not accept a separate migration image. The runner uses
the same idempotent, additive schema authority as admin-api startup, holds one DB connection, and
releases it before the Job exits.
